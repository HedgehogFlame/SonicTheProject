...
// aerial acceleration
if input_check(player_id, cLEFT)
{
    // set facing
    facing = -1;
...
if input_check(player_id, cRIGHT)
{
    // set facing
    facing = 1;
...
// flight height
if input_check_pressed(player_id, cACTION) and alarm[0] and yspeed>=flight_threshold flight_force = -0.125;
...
if not alarm[0] {if not audio_is_playing(sndFlightEnd) {audio_stop_sound(reserved_sound); reserved_sound = audio_play_sound_on(audio, sndFlightEnd, true, 0);}} else
if not audio_is_playing(sndFlight) {audio_stop_sound(reserved_sound); reserved_sound = audio_play_sound_on(audio, sndFlight, true, 0);}
